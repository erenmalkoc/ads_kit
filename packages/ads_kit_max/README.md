...
AppLovin MAX implementation of the [`ads_kit`](../ads_kit) `AdProvider`
...
`ads_kit`'s `AdProvider`/`AdManager`/`AdEvent` types.
...
`test/max_mappers_test.dart` covers every MAX -> `ads_kit` type
