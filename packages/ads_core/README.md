...
code depends only on `ads_core` — never on a mediation SDK directly — so
...
[`ads_levelplay`](https://pub.dev/packages/ads_levelplay) for Unity
LevelPlay, [`ads_max`](https://pub.dev/packages/ads_max) for AppLovin MAX)
and translate their SDK's types into this package's vocabulary. **No ad-SDK
type ever crosses into `ads_core`'s public API.**
