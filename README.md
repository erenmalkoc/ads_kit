...
Mediation platforms change: policies shift, accounts get reviewed, eCPMs
move, better deals appear. An app hard-wired to one vendor's SDK pays for
any of that with a rewrite and a store release. Behind this abstraction,
switching providers — Unity LevelPlay today, AppLovin MAX tomorrow — costs
...
```
packages/
  ads_kit/        # AdProvider contract, AdManager, FrequencyGuard,
                    # HealthMonitor, remote-config parsing, NoopAdProvider.
                    # Zero dependencies on any ad network SDK.
  ads_kit_levelplay/    # LevelPlayAdProvider — wraps unity_levelplay_mediation.
  ads_kit_max/          # MaxAdProvider — wraps applovin_max.
...
**No public API in `ads_kit` may reference an ad-SDK type.** No `AdSize`,
`AdRequest`, `MaxAd`, `IronSourceAdInfo` — nothing from a mediation SDK
crosses into `ads_kit`. Every provider package owns its own translation
to/from `ads_kit`'s types (`AdFormat`, `AdBannerSize`, `AdEvent`, `AdError`,
